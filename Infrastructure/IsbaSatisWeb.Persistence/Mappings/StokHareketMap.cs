...
namespace IsbaSatisWeb.Infrastructure.Persistence.Mappings
{
    public class StokHareketMap : IEntityTypeConfiguration<StokHareket>
    {
        public void Configure(EntityTypeBuilder<StokHareket> builder)
        {
            ...
            builder.HasOne(c => c.Stok).WithMany(c => c.StokHareketler).HasForeignKey(c => c.StokId);
            builder.HasOne(c => c.Depo).WithMany(c => c.StokHareketler).HasForeignKey(c => c.DepoId);
        }
    }
}
